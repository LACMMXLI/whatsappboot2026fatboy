...
| Motor de bot | ✅ Bastante desarrollado | Máquina de estados de conversación (saludo → nombre → promos → categorías → menú → armado de pedido → confirmación), detección de intención por palabra clave configurable por negocio, plantillas de texto editables, catálogo dinámico con alias. **Nuevo (2026-08-07):** flujos personalizados por negocio (`BotFlow`/`BotFlowStep`) — el admin crea desde el CRM sus propios mini-diálogos (horarios, ubicación, FAQs) con frase disparadora y pasos con opciones, sin tocar código. 100% basado en reglas (sin IA/LLM), aditivo y aislado del flujo de pedidos: solo se activa en reposo (`IDLE`/`ORDER_CREATED`), nunca puede interrumpir un pedido en curso. |
